    ...
    if form.validate_on_submit():
        ...
        if form.image_file.data:
            ...
            if image and allowed_image(image.filename):
                ...
                if image_path and image_path.startswith('images/') and os.path.exists(os.path.join(IMAGE_FOLDER, image_path.split('/')[-1])):
                    os.remove(os.path.join(IMAGE_FOLDER, image_path.split('/')[-1]))
